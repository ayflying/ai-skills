...
const (
	...
	CurrentModel      = "opencode/mimo-v2-omni-free"
...
func OnChatBotMessageReceived(ctx context.Context, data *chatbot.BotCallbackDataModel) ([]byte, error) {
	...
	if strings.HasPrefix(content, "/") {
		...
		case "/status":
			...
			reply := fmt.Sprintf("当前模式: %s\n模型: %s\n会话: %s", status, CurrentModel, chatTitle)
			chatbot.NewChatbotReplier().SimpleReplyText(ctx, data.SessionWebhook, []byte(reply))
		case "/opencode":
			userSession.IsInOpenCodeMode = true
			saveUserSessionsSync()
			sendMarkdown(ctx, data.SessionWebhook, "OpenCode模式", fmt.Sprintf("### 🤖 已进入OpenCode模式\n\n- **当前模型**: `%s`\n- **会话**: `%s`\n\n发送 `/exit` 退出。", CurrentModel, chatTitle))
		case "/exit":
			userSession.IsInOpenCodeMode = false
			saveUserSessionsSync()
			chatbot.NewChatbotReplier().SimpleReplyText(ctx, data.SessionWebhook, []byte("已退出OpenCode模式"))
		}
		return []byte(""), nil
	}
